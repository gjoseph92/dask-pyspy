...
# def add_decorator(
#     name: str, scheduler: distributed.Scheduler, decorator: Callable[[Func], Func]
# ) -> Callable[[], None]:
#     # if name.startswith("handler:"):
#     #     return add_decorator_scheduler_handler(
#     #         name[len("handler:") :], scheduler, decorator
#     #     )

#     obj_name, attr = name.rsplit(".", 1)
#     if obj_name == "distributed.Scheduler":
#         return add_decorator_scheduler_method(attr, scheduler, decorator)
#     return add_decorator_name(obj_name, attr, decorator)


# def add_decorator_name(
#     obj_name: str, attr: str, decorator: Callable[[Func], Func]
# ) -> Callable[[], None]:
#     obj = _importer(obj_name)
#     type_obj = obj if isinstance(obj, type) else type(obj)
#     original = getattr(type_obj, attr)
#     assert callable(original), f"{obj_name}.{attr} is not callable: {original!r}"
#     patched = decorator(original)
#     setattr(type_obj, attr, patched)
...
# def add_decorator_scheduler_method(
#     attr: str, scheduler: distributed.Scheduler, decorator: Callable[[Func], Func]
# ):
#     original = getattr(scheduler, attr)
#     assert callable(original), f"{attr} is not callable: {original!r}"
#     patched = decorator(original)
#     setattr(scheduler, attr, patched)

#     # Check if the method is a handler; if so, patch it there too
#     handler_name = None
#     handler_dict = None
#     original_func = original.__func__  # https://stackoverflow.com/a/15977850
#     for hd in (scheduler.stream_handlers, scheduler.handlers):
#         for k, v in hd.items():
#             if getattr(v, "__func__", None) is original_func:
#                 handler_name = k
#                 handler_dict = hd
#                 hd[k] = patched
#                 break
...
# def add_decorator_scheduler_handler(
#     handler: str, scheduler: distributed.Scheduler, decorator: Callable[[Func], Func]
# ):
#     handler_dict = scheduler.stream_handlers
#     try:
#         original = handler_dict[handler]
#     except KeyError:
#         handler_dict = scheduler.handlers
#         original = handler_dict[handler]

#     handler_dict[handler] = decorator(original)

#     def unpatch() -> None:
#         handler_dict[handler] = original
